...
- **Local accounts and permissions**: Built-in login, user management, task ownership, and adult-content access control.
...
- Local accounts, user management, task ownership, and adult-content permissions
...
Built-in accounts only control access inside Media Pilot. They do not replace HTTPS, network access restrictions, or permissions configured in media services such as Jellyfin.
...
On the first Web UI visit, Media Pilot requires creation of the single initial administrator and signs that account in immediately. There is no default account, default password, or self-registration.
...
The account system supports multiple operators sharing one download area, media library, and global configuration. It is not multi-tenant library isolation and does not replace final viewing permissions in services such as Jellyfin.

- The system has one permanent administrator. The administrator creates all later accounts from User Management, and those accounts are always regular users.
- The administrator can manage users, global settings, and every task. Regular users can normally see and operate only tasks they created.
- Adult-content access is controlled separately for each regular user. The sole administrator always has adult access.
- Login sessions last 30 days and are renewed during normal use. Logout, account disablement, or password changes invalidate affected sessions.
- The administrator resets forgotten passwords for regular users. A forgotten initial-administrator password is reset with the terminal command in [Deployment and Troubleshooting](docs/deployment.md#初始管理员密码恢复).
