...
首次启动且数据库中尚无用户时，访问 `/app/` 会进入初始管理员创建页面。该页面只在系统没有任何用户时开放，成功创建后立即关闭初始化入口并建立管理员会话；系统不提供默认账号或默认密码。

请先在可信的局域网或仅管理员可访问的反向代理规则下完成初始化，再向其他用户开放服务。不要在尚未创建初始管理员时把实例直接暴露到公网。初始管理员是系统中唯一且永久的管理员，固定拥有成人内容权限；后续账号只能由该管理员在“用户管理”页面创建。
