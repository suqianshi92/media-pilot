...
- **本地账号与权限**：内置登录、用户管理、任务归属和成人内容权限控制。
...
内置账号只负责 Media Pilot 应用内的访问控制，不能替代 HTTPS、网络访问限制和 Jellyfin 等媒体服务自身的权限配置。
...
首次访问 Web UI 时，系统会要求创建唯一的初始管理员并立即登录；系统不提供默认账号、默认密码或用户自行注册入口。

Prowlarr API Key 与 qBittorrent WebUI 密码由 `media-pilot-init` 自动生成并写入共享 secrets。排障和手动接管见 [部署与排障](docs/deployment.md)。

## 账号与权限

账号系统面向共享同一套下载目录、媒体库和系统设置的多操作者，不是多租户媒体库隔离，也不替代 Jellyfin 等媒体服务的最终观看权限。

- 系统只有一个永久管理员；后续账号由管理员在“用户管理”页面创建，角色固定为普通用户。
- 管理员可以管理用户、全局设置及全部任务；普通用户默认只能查看和操作自己创建的任务。
- 普通用户的成人内容权限由管理员单独控制；唯一管理员固定拥有成人内容权限。
- 登录会话有效期为 30 天，正常访问会续期；退出、停用账号或修改密码会使相关会话失效。
- 普通用户忘记密码时由管理员重置；初始管理员忘记密码时使用[部署与排障](docs/deployment.md#初始管理员密码恢复)中的终端命令。
