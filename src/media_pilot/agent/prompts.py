...
- If the filename or source directory looks like an adult production code (for example `ABCD-123`, `ABCD 123`, `ABCD_123`, or similar short-letter + number codes), prefer provider="tpdb" with media_type="movie" before treating the code as a TMDB movie title. Do not keep searching TMDB with fragments of the code if those searches return no candidates.
